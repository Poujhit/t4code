...
interface WorkspaceWorkbenchStoreState {
  ...
  pendingRevealTargetByThreadId: Record<
    ThreadId,
    WorkspaceMatchRevealTarget | null
  >;
  ...
  updateSearchState: (
    threadId: ThreadId,
    patch: Partial<WorkspaceSearchState>,
  ) => void;
  ...
  setFileError: (
    threadId: ThreadId,
    path: string,
    error: WorkspaceFileErrorState | null,
  ) => void;
  setAiReviewState: (
    threadId: ThreadId,
    path: string,
    input: WorkspaceAiReviewState,
  ) => void;
  acceptAiReviewHunk: (
    threadId: ThreadId,
    path: string,
...
export function workspaceFileStateKey(
  threadId: ThreadId,
  path: string,
): string {
...
export function workspaceAiReviewKey(
  threadId: ThreadId,
  path: string,
  turnId: TurnId,
): string {
...
function uniqueSortedPaths(paths: readonly string[]): string[] {
  return [...new Set(paths.filter((path) => path.trim().length > 0))].toSorted(
...
function normalizeThreadState(
  ...
  const normalizedSelectedPath =
    ...
    state.selectedPath.trim().length > 0
      ? state.selectedPath
      : null;
  const normalizedRootPath =
    typeof state.rootPath === 'string' && state.rootPath.trim().length > 0
      ? state.rootPath
      : null;

  const nextSelectedPath =
    normalizedRootPath === null ? null : normalizedSelectedPath;
  const nextExpandedPaths =
    normalizedRootPath === null ? [] : normalizedExpanded;
  ...
  if (
    ...
    nextExpandedPaths.every(
      (path, index) => path === state.expandedDirectoryPaths[index],
    )
  ) {
    return state;
  }
...
function updateThreadStateByThreadId(
  ...
  const isDefault =
    ...
    next.expandedDirectoryPaths.length ===
      DEFAULT_THREAD_STATE.expandedDirectoryPaths.length;
  ...
  if (
    ...
    existing.expandedDirectoryPaths.length ===
      next.expandedDirectoryPaths.length &&
...
export function clampWorkspacePaneWidth(
  width: number,
  reservedWidth = 0,
): number {
  if (typeof window === 'undefined') {
    return Math.max(
      ...
      Math.min(width, WORKSPACE_INLINE_MAX_WIDTH),
    );
  }
...
export function partializeWorkspaceWorkbenchState(
  state: WorkspaceWorkbenchStoreState,
) {
  return {
...
export function mergeWorkspaceWorkbenchPersistedState(
  ...
  return {
    ...
    threadStateByThreadId: isRecord(persistedState.threadStateByThreadId)
      ? (persistedState.threadStateByThreadId as Record<
          ThreadId,
          WorkspaceThreadState
        >)
    ...
    activeFilePathByThreadId: isRecord(persistedState.activeFilePathByThreadId)
      ? (persistedState.activeFilePathByThreadId as Record<
          ThreadId,
          string | null
        >)
      : currentState.activeFilePathByThreadId,
    draftContentByThreadIdAndPath: isRecord(
      persistedState.draftContentByThreadIdAndPath,
    )
      ? (persistedState.draftContentByThreadIdAndPath as Record<string, string>)
      : currentState.draftContentByThreadIdAndPath,
    baseMtimeMsByThreadIdAndPath: isRecord(
      persistedState.baseMtimeMsByThreadIdAndPath,
    )
    ...
    lastLoadErrorByThreadIdAndPath: isRecord(
      persistedState.lastLoadErrorByThreadIdAndPath,
    )
      ? (persistedState.lastLoadErrorByThreadIdAndPath as Record<
          string,
          WorkspaceFileErrorState
        >)
      : currentState.lastLoadErrorByThreadIdAndPath,
    aiReviewStateByThreadIdAndPath: isRecord(
      persistedState.aiReviewStateByThreadIdAndPath,
    )
      ? (persistedState.aiReviewStateByThreadIdAndPath as Record<
          string,
          WorkspaceAiReviewState
        >)
      : currentState.aiReviewStateByThreadIdAndPath,
    acceptedAiReviewHunksByKey: isRecord(
      persistedState.acceptedAiReviewHunksByKey,
    )
...
export const useWorkspaceWorkbenchStore =
  create<WorkspaceWorkbenchStoreState>()(
    persist(
      (set) => ({
        hasHydrated: false,
        ...
        setHasHydrated: (hydrated) =>
          set((state) =>
            state.hasHydrated === hydrated ? state : { hasHydrated: hydrated },
          ),
        setWorkspaceOpen: (open) =>
          set((state) =>
            state.isWorkspaceOpen === open ? state : { isWorkspaceOpen: open },
          ),
        toggleWorkspaceOpen: () =>
        ...
        setWorkspacePaneWidth: (width, reservedWidth = 0) =>
          set((state) => {
            ...
            return state.workspacePaneWidth === nextWidth
              ? state
              : { workspacePaneWidth: nextWidth };
          }),
        clampWorkspacePaneWidthToViewport: (reservedWidth = 0) =>
          set((state) => {
            const nextWidth = clampWorkspacePaneWidth(
        ...
        syncThreadRoot: (threadId, rootPath) =>
          set((state) => {
            ...
            return {
              ...
              paneModeByThreadId: deleteThreadValue(
                state.paneModeByThreadId,
                threadId,
              ),
              searchStateByThreadId: deleteThreadValue(
                state.searchStateByThreadId,
                threadId,
              ),
              openFilePathsByThreadId: deleteThreadValue(
                state.openFilePathsByThreadId,
                threadId,
              ),
              activeFilePathByThreadId: deleteThreadValue(
        ...
        focusSearchPane: (threadId) =>
          set((state) => {
            const current =
              state.searchStateByThreadId[threadId] ?? DEFAULT_SEARCH_STATE;
            return {
              paneModeByThreadId:
              ...
              searchStateByThreadId: {
                ...
                [threadId]: {
                  ...
                  collapsedFilePaths: isCollapsed
                    ? current.collapsedFilePaths.filter(
                        (entry) => entry !== path,
                      )
        ...
        requestEditorFind: (threadId) =>
          set((state) => ({
            editorFindRequestKeyByThreadId: {
              ...state.editorFindRequestKeyByThreadId,
              [threadId]:
                (state.editorFindRequestKeyByThreadId[threadId] ?? 0) + 1,
            },
          })),
              ...
              target === null
                ...
                : {
                    ...
                    [threadId]: {
                      ...target,
                      requestKey:
                        (state.pendingRevealTargetByThreadId[threadId]
                          ?.requestKey ?? 0) + 1,
                    },
                  },
          })),
        ...
        openFile: (threadId, path) =>
          set((state) => {
            const currentOpenFilePaths =
              state.openFilePathsByThreadId[threadId] ?? [];
            const nextOpenFilePaths = appendUniquePath(
        ...
        closeFile: (threadId, path) =>
          set((state) => {
            ...
            const activeFilePath =
              state.activeFilePathByThreadId[threadId] ?? null;
            const nextActiveFilePath =
              activeFilePath !== path
                ? activeFilePath
                : (nextOpenFilePaths[closeIndex] ??
                  nextOpenFilePaths[closeIndex - 1] ??
                  null);
            ...
            threadStateByThreadId: updateThreadStateByThreadId(
              ...
              (current) => ({
                ...
                expandedDirectoryPaths: expanded
                  ...
                  : current.expandedDirectoryPaths.filter(
                      (entry) => entry !== path,
                    ),
              }),
            ),
          ...
          set((state) => {
            ...
            return {
              ...
              isDirtyByThreadIdAndPath: isDirty
                ? setThreadScopedValue(
                    state.isDirtyByThreadIdAndPath,
                    threadId,
                    path,
                    true,
                  )
                : deleteThreadScopedValue(
                    state.isDirtyByThreadIdAndPath,
                    threadId,
                    path,
                  ),
        ...
        acceptAiReviewHunk: (threadId, path, hunkId) =>
          set((state) => {
            ...
            if (
              ...
              current.acceptedHunkIds.includes(hunkId)
            ) {
              return state;
            }
            const acceptedHunkIds = [...current.acceptedHunkIds, hunkId];
            const allAccepted = current.hunks.every((hunk) =>
              acceptedHunkIds.includes(hunk.id),
            );
            const acceptanceKey = workspaceAiReviewKey(
              threadId,
              path,
              current.turnId,
            );
        ...
        clearThreadState: (threadId) =>
          set((state) => ({
            ...
            activeFilePathByThreadId: Object.fromEntries(
              Object.entries(state.activeFilePathByThreadId).filter(
                ([key]) => key !== threadId,
              ),
